...
bool	iarduino_MB_Pump::setFrequency(uint8_t num, uint16_t a, uint16_t b, uint16_t c){									//	num - PUMP_X/PUMP_ALL, a,b,c - частота в Гц (1-10000).
			objModbus->codeError=ERROR_SYNTAX; if( !valID ){ return false; }												//	Ошибка синтаксиса, модуль не инициализирован.
			if( num==PUMP_A && sumPUMP>=1 ){ return MB_writeAO(AO_FREQ_A, a); }else											//	Устанавливаем частоту ШИМ насоса A записав значение a в регистр "Holding Register" AO[AO_FREQ_A]. Функция возвращает 1 при успехе, 0 при неудаче.
			if( num==PUMP_B && sumPUMP>=2 ){ return MB_writeAO(AO_FREQ_B, a); }else											//	Устанавливаем частоту ШИМ насоса B записав значение a в регистр "Holding Register" AO[AO_FREQ_B]. Функция возвращает 1 при успехе, 0 при неудаче.
			if( num==PUMP_C && sumPUMP>=3 ){ return MB_writeAO(AO_FREQ_C, a); }else											//	Устанавливаем частоту ШИМ насоса C записав значение a в регистр "Holding Register" AO[AO_FREQ_C]. Функция возвращает 1 при успехе, 0 при неудаче.
...
int32_t	iarduino_MB_Pump::getFrequency(uint8_t num){																		//	num - PUMP_X.
			objModbus->codeError=ERROR_SYNTAX; if( !valID ){ return -1; }													//	Ошибка синтаксиса, модуль не инициализирован.
			if( num==PUMP_A && sumPUMP>=1 ){ return MB_readAO(AO_FREQ_A); }else												//	Читаем частоту ШИМ насоса A из регистра "Holding Register" AO[AO_FREQ_A]. Функция возвращает прочитанное значение (0...65535), или -1 при неудаче.
			if( num==PUMP_B && sumPUMP>=2 ){ return MB_readAO(AO_FREQ_B); }else												//	Читаем частоту ШИМ насоса B из регистра "Holding Register" AO[AO_FREQ_B]. Функция возвращает прочитанное значение (0...65535), или -1 при неудаче.
			if( num==PUMP_C && sumPUMP>=3 ){ return MB_readAO(AO_FREQ_C); }													//	Читаем частоту ШИМ насоса C из регистра "Holding Register" AO[AO_FREQ_C]. Функция возвращает прочитанное значение (0...65535), или -1 при неудаче.
...
bool	iarduino_MB_Pump::setPower(uint8_t num, uint8_t a, uint8_t b, uint8_t c){											//	num - PUMP_X/PUMP_ALL, a,b,c - мощность (1-255).
			objModbus->codeError=ERROR_SYNTAX; if( !valID ){ return false; }												//	Ошибка синтаксиса, модуль не инициализирован.
			if( num==PUMP_A && sumPUMP>=1 ){ return MB_writeAO(AO_PWM_A, a); }else											//	Устанавливаем мощность насоса A записав ШИМ a в регистр "Holding Register" AO[AO_PWM_A]. Функция возвращает 1 при успехе, 0 при неудаче.
			if( num==PUMP_B && sumPUMP>=2 ){ return MB_writeAO(AO_PWM_B, a); }else											//	Устанавливаем мощность насоса B записав ШИМ a в регистр "Holding Register" AO[AO_PWM_B]. Функция возвращает 1 при успехе, 0 при неудаче.
			if( num==PUMP_C && sumPUMP>=3 ){ return MB_writeAO(AO_PWM_C, a); }else											//	Устанавливаем мощность насоса C записав ШИМ a в регистр "Holding Register" AO[AO_PWM_C]. Функция возвращает 1 при успехе, 0 при неудаче.
...
int16_t	iarduino_MB_Pump::getPower(uint8_t num){																			//	num - PUMP_X.
			objModbus->codeError=ERROR_SYNTAX; if( !valID ){ return -1; }													//	Ошибка синтаксиса, модуль не инициализирован.
			if( num==PUMP_A && sumPUMP>=1 ){ return MB_readAO(AO_PWM_A); }else												//	Читаем ШИМ насоса A из регистра "Holding Register" AO[AO_PWM_A]. Функция возвращает прочитанное значение (0...65535), или -1 при неудаче.
			if( num==PUMP_B && sumPUMP>=2 ){ return MB_readAO(AO_PWM_B); }else												//	Читаем ШИМ насоса B из регистра "Holding Register" AO[AO_PWM_B]. Функция возвращает прочитанное значение (0...65535), или -1 при неудаче.
			if( num==PUMP_C && sumPUMP>=3 ){ return MB_readAO(AO_PWM_C); }													//	Читаем ШИМ насоса C из регистра "Holding Register" AO[AO_PWM_C]. Функция возвращает прочитанное значение (0...65535), или -1 при неудаче.
			return -1;																										//	Если запрошены состояния всех насосов, возвращаем ошибку. Данная операция не поддерживается.
}																															//
																															//
...
bool	iarduino_MB_Pump::setState(uint8_t num, bool a, bool b, bool c){													//	num - PUMP_X/PUMP_ALL, a,b,c - состояния true/false.
			objModbus->codeError=ERROR_SYNTAX; if( !valID ){ return false; }												//	Ошибка синтаксиса, модуль не инициализирован.
			if( num==PUMP_A && sumPUMP>=1 ){ return MB_writeDO(DO_PUMP_A, a); }else											//	Управляем насосом A записав значение a в регистр "Coil" DO[DO_PUMP_A]. Функция возвращает 1 при успехе, 0 при неудаче.
			if( num==PUMP_B && sumPUMP>=2 ){ return MB_writeDO(DO_PUMP_B, a); }else											//	Управляем насосом B записав значение a в регистр "Coil" DO[DO_PUMP_B]. Функция возвращает 1 при успехе, 0 при неудаче.
			if( num==PUMP_C && sumPUMP>=3 ){ return MB_writeDO(DO_PUMP_C, a); }else											//	Управляем насосом C записав значение a в регистр "Coil" DO[DO_PUMP_C]. Функция возвращает 1 при успехе, 0 при неудаче.
...
int8_t	iarduino_MB_Pump::getState(uint8_t num){																			//	num - PUMP_X/PUMP_ALL.
			objModbus->codeError=ERROR_SYNTAX; if( !valID ){ return -1; }													//	Ошибка синтаксиса, модуль не инициализирован.
			if( num==PUMP_A && sumPUMP>=1 ){ return MB_readDO(DO_PUMP_A); }else												//	Читаем состояние насоса A из регистра "Coil" DO[DO_PUMP_A]. Функция возвращает прочитанное значение (0/1), или -1 при неудаче.
			if( num==PUMP_B && sumPUMP>=2 ){ return MB_readDO(DO_PUMP_B); }else												//	Читаем состояние насоса B из регистра "Coil" DO[DO_PUMP_B]. Функция возвращает прочитанное значение (0/1), или -1 при неудаче.
			if( num==PUMP_C && sumPUMP>=3 ){ return MB_readDO(DO_PUMP_C); }else												//	Читаем состояние насоса C из регистра "Coil" DO[DO_PUMP_C]. Функция возвращает прочитанное значение (0/1), или -1 при неудаче.
			if( num==PUMP_ALL             ){																				//	Если запрошены состояния всех насосов...
				if( MB_requestFrom(COILS, DO_PUMP_A, sumPUMP) != sumPUMP ){ return -1; }									//	Читаем регистры "Coils" начиная с регистра DO[DO_PUMP_A], всего sumPUMP регистров. Функция возвращает количество прочитанных значений, или 0 при неудаче.
				uint8_t i=0;																								//
...
bool	iarduino_MB_Pump::setTimeOn(uint8_t num, float a, float b, float c){												//	num - PUMP_X/PUMP_ALL, a,b,c - время в секундах (0,0-6553,5).
			...
			if( num==PUMP_A && sumPUMP>=1 ){ return MB_writeAO(AO_TMR_A, (uint16_t)(a*10)); }else							//	Включаем насос A записав время a*10 в регистр "Holding Register" AO[AO_TMR_A]. Функция возвращает 1 при успехе, 0 при неудаче.
			if( num==PUMP_B && sumPUMP>=2 ){ return MB_writeAO(AO_TMR_B, (uint16_t)(a*10)); }else							//	Включаем насос B записав время a*10 в регистр "Holding Register" AO[AO_TMR_B]. Функция возвращает 1 при успехе, 0 при неудаче.
			if( num==PUMP_C && sumPUMP>=3 ){ return MB_writeAO(AO_TMR_C, (uint16_t)(a*10)); }else							//	Включаем насос C записав время a*10 в регистр "Holding Register" AO[AO_TMR_C]. Функция возвращает 1 при успехе, 0 при неудаче.
...
float	iarduino_MB_Pump::getTimeOn(uint8_t num){																			//	num - PUMP_X.
			objModbus->codeError=ERROR_SYNTAX; if( !valID ){ return -1; }													//	Ошибка синтаксиса, модуль не инициализирован.
			if( num==PUMP_A && sumPUMP>=1 ){ return (float)(MB_readAO(AO_TMR_A))/10.0f; }else								//	Читаем время до отключения насоса A из регистра "Holding Register" AO[AO_TMR_A]. Функция возвращает количество секунд (0,0...6553,5), или -1 при неудаче.
			if( num==PUMP_B && sumPUMP>=2 ){ return (float)(MB_readAO(AO_TMR_B))/10.0f; }else								//	Читаем время до отключения насоса B из регистра "Holding Register" AO[AO_TMR_B]. Функция возвращает количество секунд (0,0...6553,5), или -1 при неудаче.
			if( num==PUMP_C && sumPUMP>=3 ){ return (float)(MB_readAO(AO_TMR_C))/10.0f; }									//	Читаем время до отключения насоса C из регистра "Holding Register" AO[AO_TMR_C]. Функция возвращает количество секунд (0,0...6553,5), или -1 при неудаче.
			return -1;																										//	Если запрошены состояния всех насосов, возвращаем ошибку. Данная операция не поддерживается.
}																															//
																															//
...
bool	iarduino_MB_Pump::setPomp12V(uint8_t num, bool a, bool b, bool c){													//	num - PUMP_X/PUMP_ALL, a,b,c - коррекция true/false.
			objModbus->codeError=ERROR_SYNTAX; if( !valID ){ return false; }												//	Ошибка синтаксиса, модуль не инициализирован.
			if( num==PUMP_A && sumPUMP>=1 ){ return MB_writeDO(DO_PUMP_12V_A, a); }else										//	Управляем коррекцией питания насоса A записав значение a в регистр "Coil" DO[DO_PUMP_12V_A]. Функция возвращает 1 при успехе, 0 при неудаче.
			if( num==PUMP_B && sumPUMP>=2 ){ return MB_writeDO(DO_PUMP_12V_B, a); }else										//	Управляем коррекцией питания насоса B записав значение a в регистр "Coil" DO[DO_PUMP_12V_B]. Функция возвращает 1 при успехе, 0 при неудаче.
			if( num==PUMP_C && sumPUMP>=3 ){ return MB_writeDO(DO_PUMP_12V_C, a); }else										//	Управляем коррекцией питания насоса C записав значение a в регистр "Coil" DO[DO_PUMP_12V_C]. Функция возвращает 1 при успехе, 0 при неудаче.
...
int8_t	iarduino_MB_Pump::getPomp12V(uint8_t num){																			//	Возвращает флаг наличия коррекции питания насоса: true-вкл / false-выкл, байт флагов если читаются все насосы, или -1 при провале чтения коррекции насоса.
			objModbus->codeError=ERROR_SYNTAX; if( !valID ){ return -1; }													//	Ошибка синтаксиса, модуль не инициализирован.
			if( num==PUMP_A && sumPUMP>=1 ){ return MB_readDO(DO_PUMP_12V_A); }else											//	Читаем флаг коррекции питания насоса A из регистра "Coil" DO[DO_PUMP_12V_A]. Функция возвращает прочитанное значение (0/1), или -1 при неудаче.
			if( num==PUMP_B && sumPUMP>=2 ){ return MB_readDO(DO_PUMP_12V_B); }else											//	Читаем флаг коррекции питания насоса B из регистра "Coil" DO[DO_PUMP_12V_B]. Функция возвращает прочитанное значение (0/1), или -1 при неудаче.
			if( num==PUMP_C && sumPUMP>=3 ){ return MB_readDO(DO_PUMP_12V_C); }else											//	Читаем флаг коррекции питания насоса C из регистра "Coil" DO[DO_PUMP_12V_C]. Функция возвращает прочитанное значение (0/1), или -1 при неудаче.
			if( num==PUMP_ALL             ){																				//	Если запрошены флаги наличия коррекции питания всех насосов...
				if( MB_requestFrom(COILS, DO_PUMP_12V_A, sumPUMP) != sumPUMP ){ return -1; }								//	Читаем регистры "Coils" начиная с регистра DO[DO_PUMP_12V_A], всего sumPUMP регистров. Функция возвращает количество прочитанных значений, или 0 при неудаче.
				uint8_t i=0;																								//
...
int8_t	iarduino_MB_Pump::digitalRead(uint8_t num){																			//	num - один из вариантов PUMP_SENSOR_XXX_X.
			...
			//	Читаем состояние всех датчиков одним запросом:																//
				if( MB_requestFrom(DISCRETE_INPUTS, DI_MIN_A, 6) != 6 ){ return -1; }										//	Читаем регистры "Discrete Inputs" начиная с регистра DI[DI_MIN_A], всего 6 регистров. Функция возвращает количество прочитанных значений, или 0 при неудаче.
			...
			if( num==PUMP_SENSOR_MIN_A && sumPUMP>=1 ){ return MB_readDI(DI_MIN_A); }else									//	Читаем регистр "Discrete Input" DI[DI_MIN_A]. Функция возвращает прочитанное значение (0/1), или -1 при неудаче.
			if( num==PUMP_SENSOR_MID_A && sumPUMP>=1 ){ return MB_readDI(DI_MID_A); }else									//	Читаем регистр "Discrete Input" DI[DI_MID_A]. Функция возвращает прочитанное значение (0/1), или -1 при неудаче.
			if( num==PUMP_SENSOR_MIN_B && sumPUMP>=2 ){ return MB_readDI(DI_MIN_B); }else									//	Читаем регистр "Discrete Input" DI[DI_MIN_B]. Функция возвращает прочитанное значение (0/1), или -1 при неудаче.
			if( num==PUMP_SENSOR_MID_B && sumPUMP>=2 ){ return MB_readDI(DI_MID_B); }else									//	Читаем регистр "Discrete Input" DI[DI_MID_B]. Функция возвращает прочитанное значение (0/1), или -1 при неудаче.
			if( num==PUMP_SENSOR_MIN_C && sumPUMP>=3 ){ return MB_readDI(DI_MIN_C); }else									//	Читаем регистр "Discrete Input" DI[DI_MIN_C]. Функция возвращает прочитанное значение (0/1), или -1 при неудаче.
			if( num==PUMP_SENSOR_MID_C && sumPUMP>=3 ){ return MB_readDI(DI_MID_C); }else									//	Читаем регистр "Discrete Input" DI[DI_MID_C]. Функция возвращает прочитанное значение (0/1), или -1 при неудаче.
...
/**		ФУНКЦИИ ДУБЛЁРЫ ОБЪЕКТА objModbus ДЛЯ ЧТЕНИЯ/ЗАПИСИ С НЕСКОЛЬКИХ ПОПЫТОК	**/										//
																															//
int8_t		iarduino_MB_Pump::MB_readDO		(uint16_t reg							){ uint8_t cntRW=maxRW; while(cntRW){ int8_t   i=objModbus->coilRead            (valID, reg);        if( i>-1 ){ return i;    }else{ cntRW--; } } return -1;    }
int8_t		iarduino_MB_Pump::MB_readDI		(uint16_t reg							){ uint8_t cntRW=maxRW; while(cntRW){ int8_t   i=objModbus->discreteInputRead   (valID, reg);        if( i>-1 ){ return i;    }else{ cntRW--; } } return -1;    }
int32_t		iarduino_MB_Pump::MB_readAO		(uint16_t reg							){ uint8_t cntRW=maxRW; while(cntRW){ int32_t  i=objModbus->holdingRegisterRead (valID, reg);        if( i>-1 ){ return i;    }else{ cntRW--; } } return -1;    }
int32_t		iarduino_MB_Pump::MB_readAI		(uint16_t reg							){ uint8_t cntRW=maxRW; while(cntRW){ int32_t  i=objModbus->inputRegisterRead   (valID, reg);        if( i>-1 ){ return i;    }else{ cntRW--; } } return -1;    }
uint16_t	iarduino_MB_Pump::MB_requestFrom(uint8_t  type, uint16_t reg, uint16_t n){ uint8_t cntRW=maxRW; while(cntRW){ uint16_t i=objModbus->requestFrom         (valID, type,reg,n); if( i> 0 ){ return i;    }else{ cntRW--; } } return  0;    }
uint8_t		iarduino_MB_Pump::MB_getInfo	(uint8_t  id							){ uint8_t cntRW=maxRW; while(cntRW){ uint8_t  i=objModbus->getInfo             (id);                if( i> 0 ){ return i;    }else{ cntRW--; } } return  0;    }
int32_t		iarduino_MB_Pump::MB_diagnostic	(uint16_t func, uint16_t data			){ uint8_t cntRW=maxRW; while(cntRW){ int32_t  i=objModbus->diagnostic          (valID, func, data); if( i>-1 ){ return i;    }else{ cntRW--; } } return -1;    }
bool		iarduino_MB_Pump::MB_changeID	(uint8_t  newID							){ uint8_t cntRW=maxRW; while(cntRW){        if( objModbus->changeID            (valID, newID)  ){ valID=newID;  return true; }else{ cntRW--; } } return false; }
bool		iarduino_MB_Pump::MB_writeDO	(uint16_t reg, bool val					){ uint8_t cntRW=maxRW; while(cntRW){        if( objModbus->coilWrite           (valID, reg, val)             ){ return true; }else{ cntRW--; } } return false; }
bool		iarduino_MB_Pump::MB_writeAO	(uint16_t reg, uint16_t val				){ uint8_t cntRW=maxRW; while(cntRW){        if( objModbus->holdingRegisterWrite(valID, reg, val)             ){ return true; }else{ cntRW--; } } return false; }
